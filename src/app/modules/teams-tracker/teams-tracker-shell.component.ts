...
export class TeamsTrackerShellComponent implements OnInit {
  ...
  async onTrackTeam(team: Team): Promise<void> {
    const isTeamExist = this.trackedTeams.some(
      (item: Team) => item.id === team.id
    );
    if (!isTeamExist) {
      const teamDetails: TeamDetails =
        await this.nBADataService.getTeamDetailsWithGames(team);
  ...
  onRemoveTeam(id: number): void {
    this.trackedTeams = this.trackedTeams.filter(
      (team: TeamDetails) => team.id !== id
    );
    this.nBADataService.updateTrackedTeamsStorage(this.trackedTeams);
  }
}
